...
function analyzeLocally(text) {
  ...
  // - Patterns (0.25) and Burstiness (0.25) are the strongest AI signals
  // - Vocabulary (0.20) catches narrow word selection
  // - Perplexity (0.15) and Structure (0.15) are supporting signals
  // Weights are tuned empirically to balance precision vs recall
...
async function analyzeWithAI(text, model = 'gpt-4o-mini') {
  ...
  // Limit to 4000 chars to stay within Puter.js token limits and avoid slow responses.
  // Heuristic analysis handles the full text; AI provides deep context on the sample.
...
TEXT: ${truncated}`;
  ...
  try {
    ...
    if (err.isAuthCancellation) {
      console.info('Puter.js AI analysis skipped: sign-in was cancelled.');
    } else {
 ...
 * @param {string} text
 * @param {{ useAI?: boolean, model?: string, forceRefresh?: boolean }} options
 * @returns {Promise<Object>}
 */
async function analyze(text, options = {}) {
  const { useAI = true, model = 'gpt-4o-mini', forceRefresh = false } = options;
  ...
  try {
    ...
    if (!forceRefresh) {
      aiResult = await getCachedAIResult(textHash);
    }

    if (!aiResult) {
      aiResult = await analyzeWithAI(text, model);
      if (aiResult) {
        await setCachedAIResult(textHash, aiResult);
      }
    }
...
/**
 * Fast non-cryptographic hash of a string (variant of djb2/xor).
 * Used as the cache key for AI analysis results.
 * @param {string} text
 * @returns {string}
 */
...
/**
 * Retrieve a cached AI analysis result from chrome.storage.session.
 * @param {string} textHash
 * @returns {Promise<Object|null>}
 */
function getCachedAIResult(textHash) {
  return new Promise((resolve) => {
    if (typeof chrome === 'undefined' || !chrome.storage || !chrome.storage.session) {
      resolve(null);
      return;
    }
    const key = 'ai_cache_' + textHash;
    chrome.storage.session.get([key], (result) => {
      if (chrome.runtime.lastError) {
        console.warn('Cache read failed:', chrome.runtime.lastError.message);
        resolve(null);
        return;
      }
...
/**
 * Store an AI analysis result in chrome.storage.session.
 * @param {string} textHash
 * @param {Object} aiResult
 * @returns {Promise<void>}
 */
function setCachedAIResult(textHash, aiResult) {
  return new Promise((resolve) => {
    if (typeof chrome === 'undefined' || !chrome.storage || !chrome.storage.session) {
      resolve();
      return;
    ...
    const key = 'ai_cache_' + textHash;
    chrome.storage.session.set({ [key]: aiResult }, () => {
      if (chrome.runtime.lastError) {
        console.warn('Cache write failed:', chrome.runtime.lastError.message);
      }
      resolve();
    });
  });
